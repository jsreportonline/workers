...
process.on('SIGTERM', () => {
  ...
  db().collection('tenants').updateAsync({ serverIp: process.env.ip }, { $set: { serverIp: undefined } }, { multi: true }).finally(() => {
    process.exit()
  })
})
...
const findWorker = (tenant, containerType, body, res) => {
  ...
  worker.restartPromise = db().collection('tenants').updateAsync({ name: originalTenant }, { $set: { serverIp: undefined } }).then(() => restartContainer(worker))
